...
class Model(nn.Module):
    def __init__(self, hyper_encoder, hyper_flow):
        ...
        networks = [self.vae, self.StokesEmbedding, self.SpectralEncoder, self.TimeEncoder, self.flow, self.film_angles, self.film_rotation]
        labels = ['VAE', 'Embedding', 'SpectralEncoder', 'TimeEncoder', 'Flow', 'FiLM angles', 'FiLM rotation']
    ...
    def sample(self, velocity, sini, nangles, angles, stokesi_mn, stokesi_residual, T_max, wavelength, nsamples, xyz):
        ...
        tmp = self.vae.decoder(xyz, latent_T_samples.view(-1, self.vae_latent_dim))
